...
if incus image show vpsmgr-debian-13 >/dev/null 2>&1; then
  ...
  if ! incus image copy images:debian/13 local: --alias vpsmgr-debian-13; then
...
if incus launch vpsmgr-debian-13 "$NAME"; then
  # wait until usable
...
rm -f /etc/machine-id /var/lib/dbus/machine-id 2>/dev/null || true'; then
    ...
    if incus publish "$NAME" --alias vpsmgr/debian-sshd; then
      ...
      if incus image delete vpsmgr-debian-13 >/dev/null 2>&1; then
