...
- 用户面板：电源控制、自助重装 Debian 13/AlmaLinux 9/openSUSE Leap 16/Arch Linux、域名与 SSH 密钥管理、加密便签（可导出）、时光机（检查点快照，回滚即丢弃此后的检查点）
- 多容器用户组：相邻账号按 `用户名` + `用户名-1`、`用户名-2`… 归为一组，共用同一登录与密码管理多个容器，支持容器切换与组内统一配色
- 管理员可为每个用户分配专属颜色（在用户列表中点击用户名）：该用户的「登录面板」按钮采用此颜色，管理员代客登录时其用户面板也以此颜色渲染，方便区分不同用户；用户自行登录始终显示默认主题；同一用户组共用一个颜色
...
该参数会在安装开始时要求确认；确认后写入 `net.v4_forward=false`，跳过 vpsmgr 保留端口检查，只随机选择一个面板入口端口。HAProxy 仍会安装但不会启动。以后可用 `vps config set net.v4_forward true` 恢复 IPv4 入站转发；如果只希望保留 80/443 域名转发，可设置 `vps config set net.v4_forward web-only`。HAProxy 也可通过 `vps config set net.haproxy false` 独立关闭；关闭后不会运行或自启，且不能添加新域名。（该开关原名为 `net.traefik`，取值语义不变，旧配置会被自动读取并使用。）
