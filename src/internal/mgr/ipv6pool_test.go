...
func TestPickPoolIPv6ExplicitAndUsed(t *testing.T) {
	...
	if _, err := d.CreateUserFull("alice", "h", "10.42.0.2", 1, 30001, 10000, 1, 1024, 10, 0, db.StatusReady, "2001:db8::2", 0, ""); err != nil {
		t.Fatal(err)
	}
...
func TestPickPoolIPv6Exhausted(t *testing.T) {
	...
	if _, err := d.CreateUserFull("alice", "h", "10.42.0.2", 1, 30001, 10000, 1, 1024, 10, 0, db.StatusReady, "2001:db8::1", 0, ""); err != nil {
		t.Fatal(err)
	}
	got, err := m.pickPoolIPv6("")
	if err != nil {
