...
func tr(l, key string, args ...any) string {
	m := map[string][2]string{ // key -> [zh, en]
		...
		"blocked_saved":          {"封禁域名列表已保存", "blocked-domains list saved"},
		...
		"err_cpu_limit_duration": {"时长必须有效：小时 ≥0，分钟 0..59，且启用时总时长 >0", "duration must be valid: hours >= 0, minutes 0..59, and > 0 when enabled"},
	}
	pair, ok := m[key]
	if !ok {
		return key
